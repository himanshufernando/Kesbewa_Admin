...
class ConfirmedOrdersFragment : Fragment() {
            ...
            when (response) {
                is KesbewaResult.Success -> {
                    ...
                    if (orderResponse.order_dispatch_type == "STORE") {
                        sendStoreOrderPackedUpdate(orderResponse)
                    }
    ...
    fun sendStoreOrderPackedUpdate(orderRespons: OrderRespons) {

        var orderID = orderRespons.order_code
        ...
        confrimHTML = "<p>&nbsp;</p>\n" +
                ...
                "<p style=\"font-size: 14px; line-height: 1.2; word-break: break-word; text-align: left; font-family: Montserrat, 'Trebuchet MS', 'Lucida Grande', 'Lucida Sans Unicode', 'Lucida Sans', Tahoma, sans-serif; mso-line-height-alt: 17px; margin: 0;\"><strong><span style=\"font-size: 24px;\"> Your Order is Ready for Pick Up - " + orderID + "</span></strong></p>\n" +
                "</div>\n" +
                "</div>\n" +
        ...
        sendAdminKesbewa(orderRespons)
        sendUser(orderRespons)
    ...
    fun sendUser(orderRespons: OrderRespons) {
        MaildroidX.Builder()
            ...
            .subject(orderRespons.order_code + " Order Ready for Pick Up")
            .body(confrimHTML)
            .onCompleteCallback(object : MaildroidX.onCompleteCallback {
                ...
                override fun onFail(errorMessage: String) {
                    println("ssssssssssssssssssssssssssssssss confirmd send user : "+errorMessage)
                    Toast.makeText(
                        requireContext(),
