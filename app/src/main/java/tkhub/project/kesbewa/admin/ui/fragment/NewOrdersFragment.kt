...
class NewOrdersFragment : Fragment() {
    ...
    var listNew = java.util.ArrayList<OrderRespons>()
    var orderID = ""
    var confrimHTML = ""
    ...
    companion object {
        ...
        var kOrder: DatabaseReference? = database?.getReference("KOrders")
        ...
        viewmodel.newOrders.observe(viewLifecycleOwner, Observer { response ->
            ...
            when (response) {
                is KesbewaResult.Success -> {
                    ...
                    adapter.submitList(listNew.reversed())
                }
        ...
        viewmodel.orderUpdateResponse.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is KesbewaResult.Success -> {
                    ...
                    if (orderResponse.order_status_code != 6) {
    ...
    private fun dialogOrderReject(orderRespons: OrderRespons) {
        ...
        dialogReject.layout_reject_dialog_reject.setOnClickListener {
            ...
            if (rejectResaon.isEmpty() || rejectResaon.isBlank()) {
                ...
                orderRespons.order_status_code = 6
                orderUpdate(orderRespons)
    ...
    private fun dialogOrderEdit(orderRespons: OrderRespons) {

        orderEdit = Dialog(requireContext())
        orderEdit.requestWindowFeature(Window.FEATURE_NO_TITLE)
        orderEdit.window!!.setBackgroundDrawableResource(android.R.color.transparent)
        orderEdit.setContentView(R.layout.dialog_order_edit)
        orderEdit.setCancelable(true)

        orderEdit.edt_delivery.setText(orderRespons.order_delivery_chargers.toString())
        orderEdit.edt_discount.setText(orderRespons.order_discount.toString())


        orderEdit.store.isChecked = orderRespons.order_dispatch_type != "DELIVERY"
        orderEdit.delivery.isChecked = orderRespons.order_dispatch_type != "STORE"





        orderEdit.radioGroup?.setOnCheckedChangeListener { group, checkedId ->
            if (R.id.negombo == checkedId) {
                var total = orderRespons.order_total_price - orderRespons.order_delivery_chargers
                var hashMap: HashMap<String, Any> = HashMap<String, Any>()
                hashMap["order_store_location"] = "NEGOMBO"
                hashMap["order_dispatch_type"] = "STORE"
                hashMap["order_delivery_chargers"] = 0.0
                hashMap["order_total_price"] = total
                kOrder?.child(orderRespons.order_id)?.updateChildren(hashMap)
        ...
        orderEdit.radiogroup_dispatch_type?.setOnCheckedChangeListener { group, checkedId ->
            if (R.id.store == checkedId) {
                orderEdit.cl_dispatch_store.visibility = View.VISIBLE

            } else {
                orderEdit.cl_dispatch_store.visibility = View.GONE

                var newcity = City_v2()


                for (city in citylist) {
                    if (city.name_en == orderRespons.delivery_address.user_address_city) {
                        newcity = city
        ...
        orderEdit.btn_save.setOnClickListener {
            var delivery = 0.0
            var discout = 0.0

            try {
                delivery = orderEdit.edt_delivery.text.toString().toDouble()
            } catch (num: NumberFormatException) {

            }

            try {
                discout = orderEdit.edt_discount.text.toString().toDouble()
            } catch (num: NumberFormatException) {
            ...
            when {
                (delivery != orderRespons.order_delivery_chargers) -> {
                    var total =
                        (orderRespons.order_total_price - orderRespons.order_delivery_chargers) + delivery

                    var hashMap: HashMap<String, Any> = HashMap<String, Any>()
                    hashMap["order_delivery_chargers"] = delivery

                    if (discout != orderRespons.order_discount) {
                        if (discout < total) {
                            total = 0.0
                        } else {
                            total -= discout
                ...
                discout != orderRespons.order_discount -> {
                    var total =
                        (orderRespons.order_total_price + orderRespons.order_discount) - discout

                    var hashMap: HashMap<String, Any> = HashMap<String, Any>()
                    hashMap["order_discount"] = discout
                    hashMap["order_total_price"] = total
    ...
    fun generatePDFMoreThanOnePage(orderRespons: OrderRespons) {
        ...
        canvas.drawText("UNPAID", 540.0f, 240.0f, paint)
