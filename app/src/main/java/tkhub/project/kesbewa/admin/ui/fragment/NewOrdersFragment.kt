...
class NewOrdersFragment : Fragment() {
    ...
    override fun onCreateView(
                    ...
                    R.id.textview_confrim -> {
                        IonAlert(requireContext(), IonAlert.ERROR_TYPE)
                            ...
                            .setConfirmClickListener(IonAlert.ClickListener { sDialog ->
                                ...
                                viewmodel.orderStatusUpdate(orderRespons,0,"")
        ...
        viewmodel.newOrders.observe(viewLifecycleOwner) { response ->
    ...
    fun generatePDFMoreThanOnePage(orderRespons : OrderRespons){
        ...
        val originalImg: Bitmap = BitmapFactory.decodeResource(requireContext().resources, R.drawable.ic_logo)
        canvas.drawBitmap(Bitmap.createScaledBitmap(originalImg, 150, 150, true), 30.0f, 50.0f, paint)

        try {
            val bitMatrix = MultiFormatWriter().encode(orderRespons.order_id, BarcodeFormat.QR_CODE, 200, 200)
        ...
        if((orderRespons.order_dispatch_type == "DELIVERY")){
            ...
            canvas.drawText(": "+orderRespons.delivery_address.user_address_number, 180.0f, 420.0f, paint)
            canvas.drawText("  "+orderRespons.delivery_address.user_address_one+", "+orderRespons.delivery_address.user_address_two, 180.0f, 450.0f, paint)
            canvas.drawText("  "+orderRespons.delivery_address.user_address_city, 180.0f, 480.0f, paint)
            dynimcY = 480f
        ...
        if(orderRespons.user.user_name.toString().length<20){
            ...
            canvas.drawText(orderRespons.user.user_name.toString().substring(0,17)+"...", 540.0f, 300.0f, paint)
        }
       ...
       // pdfFooterCreator(canvas,paint,orderRespons)
        ...
        try {
            val bitMatrix = MultiFormatWriter().encode(orderRespons.order_id, BarcodeFormat.QR_CODE, 100, 100)
            val createBitmap: Bitmap = BarcodeEncoder().createBitmap(bitMatrix)
    ...
    fun pdfFooterCreator(canvas : Canvas,paint : Paint,orderRespons : OrderRespons){
        ...
        if((orderRespons.order_dispatch_type == "DELIVERY")){
            ...
            canvas.drawText(": RS "+orderRespons.order_subtotal_total.toString(), 650.0f, dinamicY, paint)
        }
        ...
        canvas.drawText("Thank you for choosing kesbewa to reduce the usage of polythene and plastics", 40.0f, 1150f, paint)
